...
export function MrrArea({ data }: { data: { month: string; mrr: number; newMrr: number }[] }) {
  return (
    <ResponsiveContainer width="100%" height="100%">
      <AreaChart data={data} margin={{ top: 6, right: 6, left: -18, bottom: 0 }}>
        <defs>
          <linearGradient id="mrr" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor="#008577" stopOpacity={0.4} />
            <stop offset="100%" stopColor="#008577" stopOpacity={0} />
          </linearGradient>
        </defs>
        <CartesianGrid vertical={false} stroke={gridStroke} strokeDasharray="4 4" />
        ...
        <YAxis tick={axisStyle} axisLine={false} tickLine={false} tickFormatter={(v) => `£${v / 1000}k`} />
        <Tooltip content={<ChartTooltip formatter={(v: number) => `£${v.toLocaleString()}`} />} />
        <Area type="monotone" dataKey="mrr" name="MRR" stroke="#008577" strokeWidth={2.5} fill="url(#mrr)" />
      </AreaChart>
    </ResponsiveContainer>
  );
}
  ...
  return (
    <ResponsiveContainer width="100%" height="100%">
      <BarChart data={data} margin={{ top: 6, right: 6, left: -18, bottom: 0 }}>
        ...
        {keys.map((k) => (
          <Bar key={k.key} dataKey={k.key} name={k.label} fill={k.color} radius={[5, 5, 0, 0]} />
        ))}
      </BarChart>
    </ResponsiveContainer>
  );
